...
function verifyTypeScriptSetup() { // eslint-disable-line
    ...
    try {
        ...
        console.error(
            chalk.bold(
                ...
                'by running',
                chalk.cyan.bold( // eslint-disable-line prefer-template
        ...
        console.error(
            chalk.bold(
                'If you are not trying to use TypeScript, please remove the ' + // eslint-disable-line prefer-template
    ...
    try {
        ...
        console.error(
            chalk.red.bold(
                'Could not parse',
                chalk.cyan('tsconfig.json') + '.', // eslint-disable-line prefer-template
    ...
    for (const option of Object.keys(compilerOptions)) {
        ...
        const coloredOption = chalk.cyan('compilerOptions.' + option) // eslint-disable-line prefer-template
            ...
            if (parsedCompilerOptions[option] === undefined) {
                ...
                messages.push(
                    `${coloredOption} to be ${chalk.bold(
                        ...
                        suggested)} (this can be changed)`
                )
            }
        } else if (parsedCompilerOptions[option] !== valueToCheck) {
            appTsConfig.compilerOptions[option] = value
            messages.push(
                `${coloredOption} ${chalk.bold( // eslint-disable-line prefer-template
                    valueToCheck === null ? 'must not' : 'must'
                )} be ${valueToCheck === null ? 'set' : chalk.cyan.bold(
                    value)}` +
                (reason !== null ? ` (${reason})` : '') // eslint-disable-line no-negated-condition
            )
        }
    }
    ...
    if (messages.length > 0) {
        if (firstTimeSetup) {
            ...
            messages.forEach((message) => {
                console.warn('  - ' + message) // eslint-disable-line prefer-template
            })
            console.warn()
        }
